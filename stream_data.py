    ...
    async with qtm.TakeControl(connection, "password"):
        ...
        if realtime:
            ...
            for i in range(len(velec_order)):
                ...
                framenumber_start = framenumber

                # Pause between stimulations
                time.sleep(1)
                # Stimulate predefined velecs and set event markers
                await connection.set_qtm_event(name)
                change_events.append(len(framesOfPositions))
                velecs[i].stim(2)

                await connection.await_event()

                framenumber_stop = framenumber

                interval_time = range(framenumber_start, framenumber_stop)

                flexion_ind1 = flexion_mcp(interval_time, 'ind')
                flexion_ind2 = flexion_pip(interval_time, 'ind')
                flexion_mid1 = flexion_mcp(interval_time, 'mid')
                flexion_mid2 = flexion_pip(interval_time, 'mid')
                flexion_thumb1 = flexion_mcp(interval_time, 'thumb')
                flexion_thumb2 = flexion_pip(interval_time, 'thumb')
        ...
        ax1.plot(range(len(active_velec)), active_velec)
        plt.show()


        # Split recorded data into chunks per active velec
        sublists = []
        change_events = [10,213,408,567,798,982,1186]
        old_event = 0
        for event in change_events:
            sublists.append(all_flexions.iloc[:, old_event:event])
            old_event = event
